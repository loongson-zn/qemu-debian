...
unless(defined(&_BITS_TYPESIZES_H)) {
    ...
    if((defined(&__TIMESIZE) ? &__TIMESIZE : undef) == 64 && (defined(&__WORDSIZE) ? &__WORDSIZE : undef) == 32) {
	eval 'sub __INO_T_TYPE () { &__UQUAD_TYPE;}' unless defined(&__INO_T_TYPE);
	eval 'sub __OFF_T_TYPE () { &__SQUAD_TYPE;}' unless defined(&__OFF_T_TYPE);
	eval 'sub __RLIM_T_TYPE () { &__UQUAD_TYPE;}' unless defined(&__RLIM_T_TYPE);
	eval 'sub __BLKCNT_T_TYPE () { &__SQUAD_TYPE;}' unless defined(&__BLKCNT_T_TYPE);
	eval 'sub __FSBLKCNT_T_TYPE () { &__UQUAD_TYPE;}' unless defined(&__FSBLKCNT_T_TYPE);
	eval 'sub __FSFILCNT_T_TYPE () { &__UQUAD_TYPE;}' unless defined(&__FSFILCNT_T_TYPE);
	eval 'sub __TIME_T_TYPE () { &__SQUAD_TYPE;}' unless defined(&__TIME_T_TYPE);
	eval 'sub __SUSECONDS_T_TYPE () { &__SQUAD_TYPE;}' unless defined(&__SUSECONDS_T_TYPE);
    } else {
    ...
    if(defined (&__LP64__) || ((defined(&__TIMESIZE) ? &__TIMESIZE : undef) == 64 && (defined(&__WORDSIZE) ? &__WORDSIZE : undef) == 32)) {
	...
	eval 'sub __KERNEL_OLD_TIMEVAL_MATCHES_TIMEVAL64 () {( &__WORDSIZE == 64);}' unless defined(&__KERNEL_OLD_TIMEVAL_MATCHES_TIMEVAL64);
    } else {
